...
@section('content')
    ...
    <!-- Default Table Style -->
        <h2 class="content-heading">ジャンル編集</h2>
        <!-- Floating Labels -->
        <div class="block col-md-9">
            <div class="block-content">
                <form action="{{ route('admin.genre.edit', ['id' => $genre->id]) }}" method="post" class="js-validation-material">
                    @csrf
                        ...
                        <div class="col-5">
                            <select class="form-control" id="app_cd" name="app_cd">
                                @foreach(config('code.appCd') as $key => $appCd)
                                    <option value="{{ strtoupper($key) }}" @if(strtoupper($key) == old('app_cd', $genre->app_cd)) selected @endif>{{ $appCd[strtoupper($key)] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-3">
                            <select class="form-control" id="big_genre" name="big_genre">
                                ...
                                @foreach(config('const.genre.bigGenre') as $key => $value)
                                    <option value="{{ $key }}" @if($key == old('big_genre', $bigGenre)) selected @endif>{{ $value['value'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-3">
                            <select class="form-control" id="middle_genre" name="middle_genre">
                                ...
                                @foreach($middleGenres as $key => $value)
                                    <option value="{{ strtolower($value->genre_cd) }}" @if(strtolower($value->genre_cd) == old('middle_genre', $middleGenre)) selected @endif>{{ $value->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-3">
                            <select class="form-control" id="small_genre" name="small_genre">
                                ...
                                @foreach($smallGenres as $key => $value)
                                    <option value="{{ strtolower($value->genre_cd) }}" @if(strtolower($value->genre_cd) == old('small_genre', $smallGenre)) selected @endif>{{ $value->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="form-material">
                            <input type="text" class="form-control" id="genre_cd" name="genre_cd" value="{{old('genre_cd', $genre->genre_cd)}}" required>
                        ...
                        <div class="form-material">
                            <div class="text-right">
                                <label class="css-control css-control css-control-primary css-switch">
                                    @php
                                        $check = $genre->published ? true: false;
