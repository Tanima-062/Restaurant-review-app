...
@section('content')
    ...
    <!-- Default Table Style -->
        <h2 class="content-heading">精算会社追加</h2>
        <!-- Floating Labels -->
        <div class="block col-md-9">
            <div class="block-content">
                <form action="{{ route('admin.settlementCompany.add') }}" method="post" class="js-validation-material">
                    @csrf
                        ...
                        <div class="col-8">
                            @foreach($cycle as $v)
                                <div class="custom-control custom-radio custom-control-inline">
                                    <input class="custom-control-input" type="radio" name="payment_cycle" id="payment_cycle_{{$v['value']}}" value="{{$v['value']}}" @if($v['value'] == old('payment_cycle', 'TWICE_A_MONTH')) checked @endif>
                    ...
                    <div class="form-group row" id="cycle">
                        <div class="col-8">
                            @foreach($baseAmount as $v)
                                <div class="custom-control custom-radio custom-control-inline">
                                    <input class="custom-control-input" type="radio" name="result_base_amount" id="result_base_amount_{{$v['value']}}" value="{{$v['value']}}" @if($v['value'] == old('result_base_amount', 'TAX_INCLUDED')) checked @endif>
                    ...
                    <div class="form-group row" id="cycle">
                        <div class="col-8">
                            @foreach($taxCalculation as $v)
                                <div class="custom-control custom-radio custom-control-inline">
                                    <input class="custom-control-input" type="radio" name="tax_calculation" id="tax_calculation_{{$v['value']}}" value="{{$v['value']}}" @if($v['value'] == old('tax_calculation', 'EXCLUSIVE')) checked @endif>
                    ...
                    <div class="form-group">
                        <div class="form-material" style="padding-top: 40px">
                            <input type="text" class="form-control" id="branch_number" name="branch_number" value="{{old('branch_number')}}">
                    ...
                    <div class="form-group row" id="type">
                        <div class="col-8">
                            @foreach($accountType as $v)
                                <div class="custom-control custom-radio custom-control-inline">
                                    <input class="custom-control-input" type="radio" name="account_type" id="account_type_{{$v['value']}}" value="{{$v['value']}}" @if($v['value'] == old('account_type', 'SAVINGS')) checked @endif>
                    ...
                    <div class="form-group">
                        <div class="form-material" style="padding-top: 40px">
                            <input type="text" class="form-control" id="account_name_kana" name="account_name_kana" value="{{old('account_name_kana')}}">
                            <label for="account_name_kana">口座名義カナ
                                <span class="badge bg-danger ml-2 text-white">必須</span>
                                <small class="d-block text-danger text-secondary">&#x203B; 口座名義カナが未定の方は、-（半角ハイフン）で登録してください</small>
                            </label>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="form-material">
                            <input type="text" class="form-control" id="billing_email_1" name="billing_email_1" value="{{old('billing_email_1')}}" required>
                            <label for="billing_email_1">通知書送付先メールアドレス1<span class="badge bg-danger ml-2 text-white">必須</span></label>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="form-material">
                            <input type="text" class="form-control" id="billing_email_2" name="billing_email_2" value="{{old('billing_email_2')}}">
