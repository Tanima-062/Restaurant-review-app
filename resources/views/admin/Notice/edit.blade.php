...
@section('content')
    ...
    <!-- Default Table Style -->
        <h2 class="content-heading">お知らせ編集</h2>
        <!-- Floating Labels -->
        <div class="block col-md-9">
            <div class="block-content">
                <form action="{{url('admin/notice/edit').'/'.$notice->id}}" method="post" class="js-validation-material">
                    {{csrf_field()}}
                        ...
                        <div class="form-material">
                            ...
                            <select class="form-control" id="app_cd" name="app_cd" required>
                                ...
                                @foreach(config('code.appCd') as $key => $appCd)
                                    ...
                                    <option value="{{ strtoupper($key) }}" {{ old('app_cd', $notice->app_cd) == strtoupper($key) ? 'selected' : '' }}>{{ $appCd[strtoupper($key)] }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <div class="form-material">
                            <textarea class="form-control" id="message" name="message" rows="3" required>{{old('message', $notice->message)}}</textarea>
                    ...
                    <div class="form-group row">
                        <div class="col-3">
                            <input type="text" class="form-control" id="published_at" name="published_at" value="{{old('published_at', $notice->published_at)}}" autocomplete="off" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group row" id="datetime">
                        <div class="col-3">
                            <input type="text" class="form-control" id="datetime_from" name="datetime_from" value="{{old('datetime_from', $notice->datetime_from)}}" autocomplete="off" required>
                        </div>
                        <span style="margin-top:5px">～</span>
                        <div class="col-3">
                            <input type="text" class="form-control" id="datetime_to" name="datetime_to" value="{{old('datetime_to', $notice->datetime_to)}}" autocomplete="off" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group row" id="ui">
                        <div class="col-12">
                            <div class="custom-control custom-checkbox mb-5">
                                @php
                                    $check = $notice->ui_website_flg ? true : false;
                            ...
                            <div class="custom-control custom-checkbox mb-5">
                                @php
                                    $check = $notice->ui_admin_flg ? true : false;
                                    if (!empty(old('redirect_to'))) {
                        ...
                        <div class="form-material">
                            <div class="text-right">
                                <label class="css-control css-control css-control-primary css-switch">
                                    @php
                                        $check = $notice->published ? true : false;
                                        if (!empty(old('redirect_to'))) {
                                            if (old('published') !== '1') {
                                                $check = false;
                                            } else {
