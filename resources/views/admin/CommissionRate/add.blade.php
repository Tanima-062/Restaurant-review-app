...
@section('content')
    ...
    <!-- Default Table Style -->
        <h2 class="content-heading">販売手数料追加</h2>
        <!-- Floating Labels -->
        <div class="block col-md-9">
            <div class="block-content">
                <form action="{{ route('admin.commissionRate.add', ['settlementCompanyId' => $settlementCompanyId]) }}" method="post" class="js-validation-material">
                    @csrf
                        ...
                        <div class="col-4">
                            <select class="form-control" id="accounting_condition" name="accounting_condition">
                                @foreach(config('const.commissionRate.accounting_condition') as $key => $accountingCondition)
                                    <option value="{{ $key }}" @if($key == old('accounting_condition')) selected @endif>{{ $accountingCondition }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group row" id="apply_term">
                        <div class="col-2">
                            <select class="form-control" id="apply_term_from_year" name="apply_term_from_year">
                                @for($i = 2021; $i <= 2099; $i++)
                                    <option value="{{$i}}" @if($i == old('apply_term_from_year')) selected @endif>{{$i}}</option>
                                @endfor
                            </select>
                        </div>
                        ...
                        <div class="col-2">
                            <select class="form-control" id="apply_term_from_month" name="apply_term_from_month">
                                @for($i = 1; $i <= 12; $i++)
                                    <option value="{{$i}}" @if($i == old('apply_term_from_month')) selected @endif>{{sprintf('%02d', $i)}}</option>
                                @endfor
                            </select>
                        </div>
                        ...
                        <div class="col-2">
                            <select class="form-control" id="apply_term_to_year" name="apply_term_to_year">
                                @for($i = 2021; $i <= 2099; $i++)
                                    <option value="{{$i}}" @if($i == old('apply_term_to_year')) selected @endif>{{$i}}</option>
                                @endfor
                            </select>
                        </div>
                        <span style="margin-top:5px">年</span>
                        <div class="col-2">
                            <select class="form-control" id="apply_term_to_month" name="apply_term_to_month">
                                @for($i = 1; $i <= 12; $i++)
                                    <option value="{{$i}}" @if($i == old('apply_term_to_month')) selected @endif>{{sprintf('%02d', $i)}}</option>
                                @endfor
                            </select>
                        </div>
                        <span style="margin-top:5px">月</span>
                    ...
                    <div class="form-group row" id="rate">
                        <div class="col-3">
                            <input type="text" class="form-control"  id="fee" name="fee" value="{{old('fee')}}" required>
                        </div>
